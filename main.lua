...
model = nn.Container():add(enc):add(dec)
...
local toTable = nn.SplitTable(1, 1):cuda()
...
for i = 1, opt.niter do
    ...
    for j = 1, shuffle:size(1) do
        ...
        local feval = function(x)
            ...
            local zeroTensor = torch.Tensor(opt.batchSize, opt.vecSize):zero():cuda()
