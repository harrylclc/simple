...
local GRUAttention, parent = torch.class('nn.GRUAttentionBN', 'nn.AbstractRecurrent')
...
function GRUAttention:buildCell()
    ...
    local i2gSeq = nn.Sequential()
    local o2gSeq = nn.Sequential()
    ...
    local bn1 = nn.BatchNormalization(2*self.outputSize)
    local bn2 = nn.BatchNormalization(2*self.outputSize)
    bn1.weight:fill(0.1)
    bn2.weight:fill(0.1)
    i2gSeq:add(self.i2g):add(bn1)
    o2gSeq:add(self.o2g):add(bn2)

    local para = nn.ParallelTable():add(i2gSeq):add(o2gSeq)
